...
class YoloService(threading.Thread):
    ...
    def processImage(self, image):
        ...
        source = image
        save_img, save_img, screenshot, save_dir, webcam = detect_queue.input_save(source, self.opt['save_txt'],
                                                                                   self.opt['nosave'],
                                                                                   self.opt['project'],
                                                                                   self.opt['name'],
                                                                                   self.opt['exist_ok'])
        self.save_img, self.save_img, self.screenshot, self.save_dir, self.webcam = save_img, save_img, screenshot, save_dir, webcam
        bs, dataset, vid_path, vid_writer = detect_queue.data_loader(self.webcam, source, self.imgsz, self.stride,
                                                                     self.pt, self.opt['vid_stride'], self.screenshot)
